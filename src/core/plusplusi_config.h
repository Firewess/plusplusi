/*
 @des: 从网上获得的读取配置文件类，https://cooker.iteye.com/blog/777455， 后期根据开发情况进行更改
 @time: 2019/2/10
*/
#pragma once
...
public:
    ...
    explicit Config(std::string filename, std::string&& delimiter = "=", std::string&& comment = "#");

    Config();

    template<class T>
    T
    Read(const std::string &in_key) const;  //!<Search for key and read value or optional default value, call as read<T>
    template<class T>
...
public:
    ...
    struct Key_not_found : public std::exception
